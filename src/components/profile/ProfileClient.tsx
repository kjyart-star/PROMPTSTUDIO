                  ...
                  <div className="mb-10">
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                      {visibleLooseTracks.slice(0, 2).map((song: any, index: number) => {
                        ...
                        return (
                          <div key={song.id || index} className="relative rounded-3xl overflow-hidden border border-white/[0.12] shadow-2xl p-6 bg-gradient-to-r from-[#1e222d] via-[#171a23] to-[#12141b] hover:border-white/25 transition-all flex flex-col sm:flex-row items-center gap-6 min-h-[180px]">
                            ...
                            <div className="flex-1 text-center sm:text-left space-y-2 min-w-0">
                              ...
                              <div className="flex items-center gap-4 text-xs text-zinc-300 font-medium pt-1 justify-center sm:justify-start">
                                <span className="flex items-center gap-1">
                                  <Play className="w-3.5 h-3.5 fill-current text-zinc-400" /> {song.form?.play_count || (song.title?.length * 450 + 1000) || '0'} Plays
                                </span>
                                <button 
                                  onClick={(e) => { e.stopPropagation(); toggleLikeSong(song); }}
                                  className={`flex items-center gap-1 hover:text-[#2ee6a8] transition-colors ${isLiked ? 'text-[#2ee6a8] font-bold' : 'text-zinc-400'}`}
                                >
                                  <Heart className={`w-3.5 h-3.5 ${isLiked ? 'fill-current text-[#2ee6a8]' : 'text-zinc-400'}`} /> {isSongLiked(song.id) ? Number(song.form?.like_count || (song.title?.length * 25 + 15) || 0) + 1 : Number(song.form?.like_count || (song.title?.length * 25 + 15) || 0)} Likes
                                </button>
                                <button
                                  ...
                                  className="flex items-center gap-1 hover:text-[#2ee6a8] transition-colors text-zinc-300 cursor-pointer"
                                  title="다운로드"
                                >
