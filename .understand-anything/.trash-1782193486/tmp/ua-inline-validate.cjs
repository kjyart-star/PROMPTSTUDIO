...
try {
  ...
  graph.nodes.forEach((n,i)=>{ if(!n.id){issues.push(`Node[${i}] missing id`);return;}
    ...
    nodeIds.add(n.id); });
  graph.edges.forEach((e,i)=>{ if(!nodeIds.has(e.source))issues.push(`Edge[${i}] source '${e.source}' not found`);
  ...
  graph.layers.forEach(l=>(l.nodeIds||[]).forEach(id=>{
    if(!nodeIds.has(id))issues.push(`Layer '${l.id}' refs missing '${id}'`);
    if(assigned.has(id))issues.push(`Node '${id}' in multiple layers`); assigned.set(id,l.id);}));
  fileNodes.forEach(id=>{if(!assigned.has(id))issues.push(`File node '${id}' not in any layer`);});
  graph.tour.forEach((s,i)=>(s.nodeIds||[]).forEach(id=>{if(!nodeIds.has(id))issues.push(`Tour[${i}] refs missing '${id}'`);}));
